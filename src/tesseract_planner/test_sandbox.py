...
# self.task = self.factory.createTaskComposerNode("FreespacePipeline3") 
# self.task = self.factory.createTaskComposerNode("OMPLPipelineAlone")

# task = factory.createTaskComposerNode("FreespaceIfoptPipeline")
# task = factory.createTaskComposerNode("TrajOptIfoptPipeline")
...
full_dlo_holding_segment_ids = [5,34] # Example # TODO: Must be passed, Default: []
...
for simplified_dlo_num_segments in range(10, max_simplified_dlo_num_segments+1):
    ...
    # Skip the intermediate waypoints for now
    ...
    # Lets keep the default profiles for now, hence comment out the following lines:
    ...
    print(f"PLANNING TOOK {planning_time} SECONDS.")
    ...
    try:
        ...
        results = AnyPoly_as_CompositeInstruction(future.context.data_storage.getData(task_output_key))
        ...
        print("Error getting the results of the global plan, Using the fallback plan")
